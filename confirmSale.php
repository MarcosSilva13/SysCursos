...
if ($id_user && $id_course && $date && ($payment != "") && $final_price && $password) {
    ...
    if (!password_verify($password, $bd_pass[0]['senha'])) { //verifica se a senha está correta
        ...
        exit();*/
        $return = ['status' => 'error', 'message' => 'Erro: Senha incorreta!'];
    } else {
        ...
        if ($dao->insertSale($obj)) {
            ...
            exit();*/
            $return = ['status' => 'ok', 'message' => 'Confirmação: Compra realizada com sucesso!'];
        } else {
            /*$_SESSION['confirm-sale-not-ok'] = true;
            header('Location: view/coursesDefault.php');
            exit();*/
            $return = ['status' => 'error', 'message' => 'Erro: Não foi possível realizar a compra!'];
        }
    }
} else {
    ...
    header('Location: view/coursesDefault.php');
    exit();*/
    $return = ['status' => 'warning', 'message' => 'Atenção: Dados insuficientes para realizar a compra!'];
}
